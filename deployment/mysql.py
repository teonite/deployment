...
def	_mysql_db_dump(filename, database, dbhost, dbuser, dbpassword, host, host_user):
	pretty_print('[+] Starting MySQL dump.')
	env.hosts = [host]
	env.user = host_user
	...
	pretty_print('[+] MySQL dump finished.')
...
def	_mysql_db_restore(filename, database, dbhost, dbuser, dbpassword, host, host_user):
	pretty_print('[+] Starting MySQL restore.')
	env.hosts = [host]
	env.user = host_user
	env.host_string = "%s@%s" %(host_user,host)
	#	env.use_ssh_config = True
	...
	pretty_print('[+] MySQL restore finished.')
...
def	_mysql_db_clone(database, dbhost, dbuser, dbpassword, host, host_user):
	pretty_print('[+] Starting MySQL clone.')
	...
	pretty_print('[+] MySQL clone finished.')
...
def	_mysql_db_migrate(database, dir, dbhost, dbuser, dbpassword, host, host_user):
	pretty_print('[+] Starting MySQL migrate')
	...
	try:
		...
		pretty_print('[+] MySQL migrate finished.')
	except:
...
def	_db_migrate(config):
	pretty_print("[+] Starting database migration.")
	...
	try:
		...
		pretty_print("[+] Database migration finished.")
	except:
		exceptionType, exceptionValue, exceptionTraceback = sys.exc_info()
		pretty_print("Something went wrong. Message: %s - %s" % (exceptionType, exceptionValue))
